...
router.get('/in', (req, res) => {
  ...
  // 获取总数
  const countSql = `SELECT COUNT(*) as total FROM stock_in si LEFT JOIN projects p ON si.project_id = p.id LEFT JOIN suppliers s ON si.supplier_id = s.id LEFT JOIN users u ON si.creator_id = u.id WHERE 1=1${keyword ? ' AND (si.stock_in_no LIKE ? OR p.name LIKE ?)' : ''}${status && status !== 'all' ? ' AND si.status = ?' : ''}${project_id ? ' AND si.project_id = ?' : ''}`;
...
router.get('/:id', (req, res, next) => {
  ...
  // 如果id是'query'、'out'、'in'、'return'等，跳过此路由
  const skipIds = ['query', 'out', 'in', 'return'];
...
router.get('/out/pending', (req, res) => {
  ...
  // 获取总数
  const countSql = sql.replace(/SELECT sa\.\*,.*?FROM/, 'SELECT COUNT(*) as total FROM');
  const countResult = db.prepare(countSql).get(...params);
  const total = countResult ? countResult.total : 0;
